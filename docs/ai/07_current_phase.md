...
- 운영 콘솔 패키지 업로드 저장 기준 정리 완료 (`2026-03-23`)
  - 파일 업로드 직후가 아니라 `패키지 업로드 저장` 시점에 intake 검증 시작
  - 실행 탭은 즉시 업로드본이 아니라 저장된 파일 기준으로 준비 상태 판단
- `company_000002` 보정테스트제약 기준 지저분한 raw intake 자동보정 검증 완료 (`2026-03-23`)
  ...
  - intake는 advisory만 남기는 수준이 아니라 Adapter가 실제로 읽을 수 있는 staging 정리본 생성까지 수행
  - Prescription `FAIL`은 통합 실행 중단 사유가 아니라 다음 단계 경고로 처리
  - Sandbox는 병원 ID 체계 보정 후 `PASS`, Territory는 실행 실패가 아니라 운영 경고 `WARN`으로 정리
- Agent run context / artifact summary 연결 보강 완료 (`2026-03-23`)
  - run 저장 시 `agent_summary`와 최신 run bundle 저장
  - Agent는 legacy summary보다 최신 run bundle / latest run history를 우선 읽음
- Territory 보고서는 로컬 라이브러리 번들 기준으로 열리도록 안정화 완료 (`2026-03-23`)
  - 현재 Territory 보고서는 `Leaflet` 기반 지도 사용
  - `leaflet.js`, `leaflet.css`, marker image를 `territory_map_preview_assets/leaflet/`에 함께 복사
  - 즉 외부 CDN이 아니라 로컬 라이브러리 기준으로 열림
- Total Validation 번들 내부 Territory 지도 렌더 보강 완료 (`2026-03-26`)
  - 개별 Territory 보고서는 정상이었고, 문제는 다운로드한 `total_valid_preview` 번들 안에서만 재현됐다
  - 통합 보고서가 Territory iframe 로드 후 지도 재계산 신호를 보내고, Territory 템플릿은 그 신호를 받아 지도 크기 재계산과 현재 선택 재렌더를 수행하도록 정리했다
  - 최종 번들 기준으로 통합 보고서 안 Territory 지도 정상 동작 확인
