...
13. `docs/architecture/16_responsibility_based_refactor_structure.md`
14. `ui/console/` 실제 구조는 wrapper가 아니라 현재 구현 기준으로 해석한다.
...
7. 현재 단계는 `docs/ai/07_current_phase.md`를 기준으로 해석한다.
8. 현재는 Part2 KPI 엔진 분리 진행 단계다.
9. run 저장은 `runs`, `run_steps`, `run_artifacts`, `run_report_context` 기준으로 본다.
10. Agent는 `run_report_context`만이 아니라 `run_artifacts`를 읽는 방향으로 해석한다.
11. 콘솔 구조는 `ui/console/` 패키지 기준으로 본다.
12. 문서는 짧고 명확한 규칙 우선으로 해석한다.
13. 저장소 실제 구조와 다른 이상적 구조를 상상하지 않는다.
