...
원천데이터를 넣으면 `정규화 -> 모듈 분석 -> OPS 검증 게이트 -> HTML 보고서 생성`까지 이어지는 SFE 운영 검증 프로젝트입니다.

지금 상태의 핵심은 이것입니다.

- 회사별 raw 데이터를 그대로 넣고 어디까지 흡수되는지 검증
- CRM, Prescription, Sandbox, Territory, Builder를 단방향 검증 흐름으로 실행
- 회사 코드별로 결과 폴더를 분리
- 최종 HTML 보고서와 통합 검증 보고서까지 생성
...
- Adapter가 먼저 정규화합니다.
- 모듈은 자기 Result Asset만 만듭니다.
- OPS는 계산기보다 `중간 검증 게이트 / 관제실` 역할에 가깝습니다.
- OPS는 매핑 상태와 품질 상태를 보고 다음 단계로 넘길지 판단합니다.
- Builder는 템플릿에 맞는 payload를 받아 HTML을 만듭니다.
- 회사가 바뀌면 공통 틀 위에서 얇게 커스텀하는 방식으로 갑니다.
...
├── ops_validation/{company_key}/   # 검증 결과 + HTML
├── public/                         # 공공 기준 데이터
...
- 완전 범용 엔진이라기보다 `회사별 커스텀 가능한 공통 틀`에 가깝습니다.
- file name 규칙은 아직 기존 검증 스크립트 기준을 일부 따릅니다.
- Prescription HTML은 경량화했지만 다른 보고서보다 여전히 무거운 편입니다.
- 통합 보고서는 생성된 개별 HTML을 한 화면에서 묶어 보는 허브입니다.
- 최종 결과 확인은 결국 템플릿이 있어야 가능하므로, 현재 구조는 `단방향 검증 확장` 기준으로 보는 것이 맞습니다.
