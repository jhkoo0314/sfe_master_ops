# OPS Project Startup Skill

## 목적

이 스킬의 목적은 새 세션이 시작될 때 Codex가
SFE OPS 프로젝트의 공식 문서 기준과 현재 작업 범위를 먼저 동기화한 뒤,
...
1. SFE OPS의 중심은 Sandbox가 아니라 OPS Engine이다.
2. 공식 흐름은 `raw -> adapter -> module -> Result Asset -> OPS -> Builder` 이다.
3. 공식 모듈 순서는 `CRM -> Prescription -> Sandbox -> Territory -> HTML Builder` 이다.
4. Builder는 계산 엔진이 아니라 최종 표현 모듈이다.
5. 모든 실행/저장 경로는 `company_key` 기준이다.
6. 현재는 Part 1 구조 고정과 문서 기준 정리 단계다.
7. 문서는 짧고 명확한 규칙 우선으로 해석한다.
8. 저장소 실제 구조와 다른 이상적 구조를 상상하지 않는다.
...
3. Sandbox를 전체 허브로 재정의하지 않는다.
4. `company_key` 구조를 무시하고 경로를 하드코딩하지 않는다.
5. 문서 기준 없는 큰 방향 전환을 하지 않는다.

---

## 한 줄 요약

이 스킬은 새 세션에서 Codex가 SFE OPS 프로젝트의 공식 기준을 먼저 읽고,
