...
13. `RUNBOOK.md`
14. `STRUCTURE.md`
15. `docs/architecture/16_responsibility_based_refactor_structure.md`
16. `ui/console/` 실제 구조는 wrapper가 아니라 현재 구현 기준으로 해석한다.
...
3. 공식 흐름은 `raw -> intake/onboarding -> _intake_staging -> adapter -> core engine/module -> result asset -> validation layer(OPS) -> intelligence(RADAR) -> builder` 이다.
...
9. 현재는 `Part2 완료` 상태다.
10. Part2 완료 선언 기준은 `docs/architecture/23_part2_completion_declaration.md`를 함께 본다.
11. run 저장은 `runs`, `run_steps`, `run_artifacts`, `run_report_context` 기준으로 본다.
12. Agent는 `run_report_context`만이 아니라 `run_artifacts`를 읽는 방향으로 해석한다.
13. 콘솔 구조는 `ui/console/` 패키지 기준으로 본다.
14. 문서는 짧고 명확한 규칙 우선으로 해석한다.
15. 저장소 실제 구조와 다른 이상적 구조를 상상하지 않는다.
16. 앞으로 단계별 작업 메모는 `docs/workstreams/`에 누적하고 `docs/part3` 같은 새 단계 폴더는 만들지 않는다.
...
4. `docs/architecture/23_part2_completion_declaration.md`
5. `docs/ai/07_current_phase.md`
6. 필요 시 루트 `README.md`, `RUNBOOK.md`, `STRUCTURE.md`
